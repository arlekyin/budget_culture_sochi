...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        # Получаем бухгалтера для обработки заявок
    ...
    def _create_users(self) -> User:
        ...
        # Сначала создаём учреждения-заглушки — они нужны для привязки
        # (реальная привязка будет после _create_institutions)
        for username, password, first_name, last_name, role, inst_idx in USERS_DATA:
            ...
            if created:
                ...
                user.save()
            # Профиль создадим/обновим после учреждений
    ...
    def _create_institutions(self, admin_user: User) -> list:
        ...
        # Теперь привязываем профили пользователей к учреждениям
    ...
    def _create_requests(self, institutions: list, admin_user: User) -> None:
        ...
        # Определяем сценарии: (учреждение_idx, статус, год, тип_финансирования,
        #                        approved_amount_ratio, comment_при_возврате)
        scenarios = [
        ...
        for (inst_idx, status, year, funding_type, ratio, comment) in scenarios:
            ...
            # Проставляем даты и согласованные суммы для финальных статусов
            if status in (RequestStatus.APPROVED, RequestStatus.INCLUDED):
                ...
                # Лог изменения суммы если бухгалтер урезал
            ...
            elif status == RequestStatus.SUBMITTED:
    ...
    def _get_lines_for_institution(self, inst_idx: int, funding_type: str) -> list:
        """Возвращает набор строк расходов, характерный для типа учреждения."""

        if funding_type == FundingType.PAID:
            return [
                {'kosgu_code': '130', 'kvr_code': '244', 'description': 'Доходы от платных образовательных услуг (оказание услуг)', 'unit_price': 3500, 'quantity': 480},
                {'kosgu_code': '290', 'kvr_code': '244', 'description': 'Расходные материалы для занятий (краски, холсты, кисти)', 'unit_price': 850, 'quantity': 120},
            ]

        # Базовые строки для всех учреждений (зарплата, коммуналка, связь)
        base = [
            {'kosgu_code': '211', 'kvr_code': '111', 'description': 'Фонд оплаты труда работников учреждения на 2026 год', 'unit_price': 1_650_000, 'quantity': 12},
            {'kosgu_code': '213', 'kvr_code': '119', 'description': 'Страховые взносы во внебюджетные фонды (30,2% от ФОТ)', 'unit_price': 498_300, 'quantity': 12},
            {'kosgu_code': '221', 'kvr_code': '244', 'description': 'Услуги телефонной связи (городская линия, 3 номера)', 'unit_price': 1_850,   'quantity': 12},
            {'kosgu_code': '221', 'kvr_code': '244', 'description': 'Услуги интернет-провайдера (тариф 100 Мбит/с)', 'unit_price': 4_200,   'quantity': 12},
            {'kosgu_code': '223', 'kvr_code': '244', 'description': 'Электроснабжение (по тарифу 5,23 руб/кВт·ч, ~8 000 кВт·ч/мес.)', 'unit_price': 41_840,  'quantity': 12},
            {'kosgu_code': '223', 'kvr_code': '244', 'description': 'Теплоснабжение (отопительный сезон октябрь–апрель)', 'unit_price': 98_400,  'quantity': 7},
            {'kosgu_code': '223', 'kvr_code': '244', 'description': 'Холодное водоснабжение и водоотведение', 'unit_price': 7_620,   'quantity': 12},
            {'kosgu_code': '225', 'kvr_code': '244', 'description': 'Техническое обслуживание систем вентиляции и кондиционирования', 'unit_price': 28_500,  'quantity': 4},
            {'kosgu_code': '290', 'kvr_code': '244', 'description': 'Уплата налога на имущество организаций (2026 год)', 'unit_price': 85_000,  'quantity': 4},
        ]

        # Специфичные строки по типу учреждения
        specific = {
            0: [  # Библиотека
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Подписка на периодические издания и базы данных (РИНЦ, e-Library)', 'unit_price': 124_500, 'quantity': 1},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Услуги по оцифровке фондов (договор с подрядчиком)', 'unit_price': 185_000, 'quantity': 1},
                {'kosgu_code': '340', 'kvr_code': '244', 'description': 'Приобретение книг и печатных изданий для пополнения фонда', 'unit_price': 650,    'quantity': 420},
                {'kosgu_code': '310', 'kvr_code': '244', 'description': 'Приобретение стеллажей для книгохранилища (7 шт. × 18 500 руб.)', 'unit_price': 18_500,  'quantity': 7},
                {'kosgu_code': '225', 'kvr_code': '244', 'description': 'Текущий ремонт кровли главного корпуса (дефектная ведомость №12)', 'unit_price': 347_000, 'quantity': 1},
            ],
            1: [  # Художественный музей
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Страхование экспонатов постоянной экспозиции (полис на 2026 г.)', 'unit_price': 218_000, 'quantity': 1},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Охрана объекта (ЧОП «Щит», 24/7, договор)', 'unit_price': 68_400,  'quantity': 12},
                {'kosgu_code': '225', 'kvr_code': '244', 'description': 'Текущий ремонт выставочного зала №3 (замена напольного покрытия)', 'unit_price': 420_000, 'quantity': 1},
                {'kosgu_code': '310', 'kvr_code': '244', 'description': 'Приобретение климатической витрины для хранения графики (1 шт.)', 'unit_price': 285_000, 'quantity': 1},
                {'kosgu_code': '340', 'kvr_code': '244', 'description': 'Расходные материалы для реставрационной мастерской', 'unit_price': 12_400,  'quantity': 8},
            ],
            2: [  # ДМШ №1
                {'kosgu_code': '225', 'kvr_code': '244', 'description': 'Настройка и техническое обслуживание 12 роялей и пианино', 'unit_price': 8_500,   'quantity': 12},
                {'kosgu_code': '310', 'kvr_code': '244', 'description': 'Приобретение цифрового пианино YAMAHA Clavinova CLP-785 (2 шт.)', 'unit_price': 198_000, 'quantity': 2},
                {'kosgu_code': '340', 'kvr_code': '244', 'description': 'Нотная литература и учебные пособия для учащихся', 'unit_price': 850,    'quantity': 280},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Аренда концертного зала «Зимний театр» для выпускного концерта', 'unit_price': 95_000,  'quantity': 2},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Услуги по охране труда и медосмотрам сотрудников', 'unit_price': 28_700,  'quantity': 1},
            ],
            3: [  # ДМШ №2
                {'kosgu_code': '225', 'kvr_code': '244', 'description': 'Техническое обслуживание и настройка 8 пианино', 'unit_price': 8_500,   'quantity': 8},
                {'kosgu_code': '310', 'kvr_code': '244', 'description': 'Приобретение скрипок (комплект 4/4) для оркестрового класса', 'unit_price': 24_500,  'quantity': 6},
                {'kosgu_code': '340', 'kvr_code': '244', 'description': 'Нотная литература и учебники по сольфеджио', 'unit_price': 720,    'quantity': 180},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Организация и проведение городского конкурса юных музыкантов', 'unit_price': 78_000,  'quantity': 1},
            ],
            4: [  # ДХШ
                {'kosgu_code': '340', 'kvr_code': '244', 'description': 'Холсты грунтованные (60×80 см), масляные краски, кисти (набор)', 'unit_price': 1_850,   'quantity': 240},
                {'kosgu_code': '310', 'kvr_code': '244', 'description': 'Мольберты деревянные напольные (20 шт. для нового класса)', 'unit_price': 4_200,   'quantity': 20},
                {'kosgu_code': '225', 'kvr_code': '244', 'description': 'Ремонт скульптурной мастерской (штукатурка, покраска стен)', 'unit_price': 210_000, 'quantity': 1},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Участие в межрегиональной выставке детского изобразительного искусства', 'unit_price': 42_500,  'quantity': 1},
            ],
            5: [  # ДК Центральный
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Организация и проведение городских мероприятий (12 ед. в год)', 'unit_price': 145_000, 'quantity': 12},
                {'kosgu_code': '225', 'kvr_code': '244', 'description': 'Техническое обслуживание звукового и светового оборудования', 'unit_price': 38_500,  'quantity': 12},
                {'kosgu_code': '310', 'kvr_code': '244', 'description': 'Приобретение микшерного пульта YAMAHA CL5 для главной сцены', 'unit_price': 685_000, 'quantity': 1},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Пошив сценических костюмов для народного ансамбля (30 комплектов)', 'unit_price': 12_800,  'quantity': 30},
                {'kosgu_code': '225', 'kvr_code': '244', 'description': 'Замена театральных кресел в зрительном зале (150 шт.)', 'unit_price': 8_400,   'quantity': 150},
            ],
            6: [  # Молодёжный театр
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Постановка нового спектакля (декорации, костюмы, реквизит)', 'unit_price': 1_850_000, 'quantity': 1},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Гонорары приглашённых режиссёров и актёров (2 постановки)', 'unit_price': 380_000, 'quantity': 2},
                {'kosgu_code': '225', 'kvr_code': '244', 'description': 'Капитальный ремонт гримёрных помещений (4 комнаты)', 'unit_price': 285_000, 'quantity': 4},
                {'kosgu_code': '310', 'kvr_code': '244', 'description': 'Приобретение профессионального светодиодного оборудования сцены', 'unit_price': 1_240_000, 'quantity': 1},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Гастроли в рамках краевого театрального фестиваля (выезд в Краснодар)', 'unit_price': 245_000, 'quantity': 1},
            ],
            7: [  # Музей истории Сочи
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Разработка и монтаж новой интерактивной экспозиции «Сочи-1864»', 'unit_price': 980_000, 'quantity': 1},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Охрана объекта (круглосуточно, договор с ЧОП)', 'unit_price': 54_200,  'quantity': 12},
                {'kosgu_code': '310', 'kvr_code': '244', 'description': 'Приобретение мультимедийного проектора Epson EB-L1500UH', 'unit_price': 384_000, 'quantity': 1},
                {'kosgu_code': '340', 'kvr_code': '244', 'description': 'Упаковочные материалы и оборудование для хранения фондов', 'unit_price': 18_400,  'quantity': 6},
                {'kosgu_code': '225', 'kvr_code': '244', 'description': 'Реставрация фасада исторического здания (лицевая часть)', 'unit_price': 1_150_000, 'quantity': 1},
            ],
            8: [  # ГКЦ
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Организация городских культурно-массовых мероприятий (план 24 ед.)', 'unit_price': 185_000, 'quantity': 24},
                {'kosgu_code': '310', 'kvr_code': '244', 'description': 'Приобретение моноблоков Apple iMac для медиастудии (5 шт.)', 'unit_price': 168_000, 'quantity': 5},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Полиграфические услуги (афиши, буклеты, программки)', 'unit_price': 34_500,  'quantity': 12},
                {'kosgu_code': '225', 'kvr_code': '244', 'description': 'Текущий ремонт малого зала (покраска стен, замена ламп)', 'unit_price': 178_000, 'quantity': 1},
                {'kosgu_code': '226', 'kvr_code': '244', 'description': 'Услуги по техническому обеспечению фестиваля «Сочи-Арт»', 'unit_price': 320_000, 'quantity': 1},
            ],
        }
    ...
    def _create_fact_payments(self, institutions: list, admin_user: User) -> None:
        ...
        # Кассовые расходы за январь–апрель 2026 года
        payments = [
            # (inst_idx, kosgu_code, amount, date, description, doc_number)
            (0, '211', Decimal('18_400_000'), datetime.date(2026, 1, 15), 'Выплата заработной платы за январь 2026 года', 'ПП-0124-01'),
            (0, '213', Decimal('5_556_800'),  datetime.date(2026, 1, 20), 'Страховые взносы за январь 2026 года', 'ПП-0125-01'),
            ...
            (0, '211', Decimal('18_400_000'), datetime.date(2026, 2, 14), 'Выплата заработной платы за февраль 2026 года', 'ПП-0199-02'),
            (0, '226', Decimal('124_500'),    datetime.date(2026, 3, 5),  'Оплата подписки на периодические издания (ООО «Пресса-Юг»)', 'ПП-0287-03'),
            (0, '211', Decimal('18_400_000'), datetime.date(2026, 3, 14), 'Выплата заработной платы за март 2026 года', 'ПП-0312-03'),
            (0, '340', Decimal('273_000'),    datetime.date(2026, 4, 8),  'Приобретение книг для пополнения библиотечного фонда (420 экз.)', 'ПП-0401-04'),
            (0, '211', Decimal('18_400_000'), datetime.date(2026, 4, 15), 'Выплата заработной платы за апрель 2026 года', 'ПП-0445-04'),

            (1, '211', Decimal('10_200_000'), datetime.date(2026, 1, 15), 'Выплата заработной платы за январь 2026 года', 'ПП-0130-01'),
            (1, '213', Decimal('3_080_400'),  datetime.date(2026, 1, 20), 'Страховые взносы за январь 2026 года', 'ПП-0131-01'),
            (1, '223', Decimal('318_400'),    datetime.date(2026, 1, 24), 'Оплата электроэнергии за январь 2026 года', 'ПП-0132-01'),
            (1, '226', Decimal('218_000'),    datetime.date(2026, 2, 3),  'Страхование музейных экспонатов на 2026 год (СК «Согаз»)', 'ПП-0201-02'),
            (1, '226', Decimal('68_400'),     datetime.date(2026, 2, 5),  'Оплата услуг охраны за февраль 2026 (ЧОП «Щит»)', 'ПП-0202-02'),
            (1, '211', Decimal('10_200_000'), datetime.date(2026, 2, 14), 'Выплата заработной платы за февраль 2026 года', 'ПП-0210-02'),
            (1, '211', Decimal('10_200_000'), datetime.date(2026, 3, 14), 'Выплата заработной платы за март 2026 года', 'ПП-0318-03'),
            (1, '211', Decimal('10_200_000'), datetime.date(2026, 4, 15), 'Выплата заработной платы за апрель 2026 года', 'ПП-0421-04'),

            (2, '211', Decimal('14_200_000'), datetime.date(2026, 1, 15), 'Выплата заработной платы за январь 2026 года', 'ПП-0140-01'),
            (2, '213', Decimal('4_288_400'),  datetime.date(2026, 1, 20), 'Страховые взносы за январь 2026 года', 'ПП-0141-01'),
            (2, '225', Decimal('102_000'),    datetime.date(2026, 2, 18), 'Настройка 12 роялей и пианино (ИП Ткаченко В.Р.)', 'ПП-0225-02'),
            (2, '211', Decimal('14_200_000'), datetime.date(2026, 2, 14), 'Выплата заработной платы за февраль 2026 года', 'ПП-0230-02'),
            (2, '310', Decimal('396_000'),    datetime.date(2026, 3, 22), 'Приобретение 2 цифровых пианино YAMAHA CLP-785 (ООО «МузТорг»)', 'ПП-0342-03'),
            (2, '211', Decimal('14_200_000'), datetime.date(2026, 3, 14), 'Выплата заработной платы за март 2026 года', 'ПП-0360-03'),
            (2, '211', Decimal('14_200_000'), datetime.date(2026, 4, 15), 'Выплата заработной платы за апрель 2026 года', 'ПП-0470-04'),

            (6, '211', Decimal('22_800_000'), datetime.date(2026, 1, 15), 'Выплата заработной платы за январь 2026 года', 'ПП-0170-01'),
            (6, '213', Decimal('6_885_600'),  datetime.date(2026, 1, 20), 'Страховые взносы за январь 2026 года', 'ПП-0171-01'),
            (6, '226', Decimal('1_850_000'),  datetime.date(2026, 2, 28), 'Аванс по договору постановки спектакля «Дядя Ваня» (реж. Коваль А.)', 'ПП-0265-02'),
            (6, '211', Decimal('22_800_000'), datetime.date(2026, 2, 14), 'Выплата заработной платы за февраль 2026 года', 'ПП-0270-02'),
            (6, '211', Decimal('22_800_000'), datetime.date(2026, 3, 14), 'Выплата заработной платы за март 2026 года', 'ПП-0380-03'),
            (6, '226', Decimal('380_000'),    datetime.date(2026, 4, 10), 'Гонорар приглашённых актёров (договор №2026-04-Г)', 'ПП-0450-04'),
            (6, '211', Decimal('22_800_000'), datetime.date(2026, 4, 15), 'Выплата заработной платы за апрель 2026 года', 'ПП-0480-04'),

            (5, '211', Decimal('16_100_000'), datetime.date(2026, 1, 15), 'Выплата заработной платы за январь 2026 года', 'ПП-0160-01'),
            (5, '213', Decimal('4_862_200'),  datetime.date(2026, 1, 20), 'Страховые взносы за январь 2026 года', 'ПП-0161-01'),
            (5, '226', Decimal('145_000'),    datetime.date(2026, 2, 20), 'Проведение мероприятия «Масленица-2026» (ДК Центральный)', 'ПП-0255-02'),
            (5, '211', Decimal('16_100_000'), datetime.date(2026, 2, 14), 'Выплата заработной платы за февраль 2026 года', 'ПП-0260-02'),
            (5, '211', Decimal('16_100_000'), datetime.date(2026, 3, 14), 'Выплата заработной платы за март 2026 года', 'ПП-0370-03'),
            (5, '211', Decimal('16_100_000'), datetime.date(2026, 4, 15), 'Выплата заработной платы за апрель 2026 года', 'ПП-0460-04'),
        ]
    ...
    def _create_reservations(self, institutions: list, admin_user: User) -> None:
        ...
        reservations = [
            ...
            (2, '226', '2026/УС-005', 'МБУК «Зимний театр»',     Decimal('190_000'),   datetime.date(2026, 3, 10), Reservation.ReservationStatus.CLOSED),
    ...
    def _print_summary(self) -> None:
        ...
        self.stdout.write('  dir_library  / dir123    — Директор ЦБС')
        self.stdout.write('  dir_museum   / dir123    — Директор Худ. музея')
        self.stdout.write('  dir_dmsh1    / dir123    — Директор ДМШ №1')
        self.stdout.write('  dir_dmsh2    / dir123    — Директор ДМШ №2')
        self.stdout.write('  dir_dhsh     / dir123    — Директор ДХШ')
        self.stdout.write('  dir_dk       / dir123    — Директор ДК Центральный')
        self.stdout.write('  dir_theater  / dir123    — Директор Молодёжного театра')
        self.stdout.write('  dir_hist     / dir123    — Директор Музея истории')
        self.stdout.write('  dir_gkc      / dir123    — Директор ГКЦ')
        self.stdout.write('')
